...
import { WebmProfile } from "./formats/WebmProfile";
...
export class RecorderEngine {
    ...
    constructor() {
        ...
        this.profiles["WEBM"] = new WebmProfile();
    }
    ...
    private convertMp4ToGif(mp4Path: string, gifPath: string): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            const args = ["-i", mp4Path, "-vf", "fps=30,scale=-1:-1:flags=lanczos,split[s0][s1];[s0]palettegen=stats_mode=diff[p];[s1][p]paletteuse", "-y", gifPath];
